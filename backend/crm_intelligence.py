...
async def run_data_extraction(phone, conv_id):
    ...
    """
    print(f"🔍 AI is extracting CRM data for {phone}...")
    ...
    if not extractor_prompt_template:
        print(" Extractor prompt not found in DB.")
        return
    ...
    try:
        ...
        if updates:
            ...
            if success:
                print(f" CRM Updated for {phone}: {updates.get('name', 'N/A')}")
            else:
                print(f" Failed to update CRM for {phone}")
                
    except Exception as e:
        print(f" Error parsing extracted data: {e}")
        print(f"Raw Response: {ai_raw_response}")
