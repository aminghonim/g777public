...
class EvolutionManager:
    ...
    def __init__(self):
        if not EVO_BASE_URL or not EVO_GLOBAL_KEY:
            ...
            logger.warning(
                "EVOLUTION_API_BASE_URL or EVOLUTION_API_GLOBAL_KEY "
                "missing from environment variables. Evolution API integrations will fail."
            )

        if EVO_BASE_URL:
            logger.info(f"Evolution API Base URL loaded: {EVO_BASE_URL}")
    ...
    async def create_instance(self, user_id: str) -> Dict[str, Any]:
        ...
        if ":3000" in self.base_url:
            logger.info("Using local Bare-metal Baileys bridge.")
