...
class BrainTrainer:
    ...
    async def train_on_questions(self, questions_list):
        ...
        print(f" Starting Training Loop for {len(questions_list)} questions...")
        ...
        for q in questions_list:
            ...
            print(f"🤖 Bot (Raw): {robotic_resp[:50]}...")
            
            # 2. تحويله لبشري
            human_resp = await self.humanize_bot_response(q, robotic_resp)
            print(f"✨ Yasmine (Human): {human_resp[:50]}...")
    ...
    async def train_on_weak_interactions(self, limit=10):
        ...
        """
        print("🔍 Scanning for weak interactions in DB...")
        with get_db_cursor() as cur:
            if not cur: return
    ...
    async def run_nightly_training(self):
        ...
        print("🌙 Starting Nightly Brain Training...")
