...
This file is the activator for the Antigravity "Full Squad Mode". When invoked, the following Iron Rules apply without exception.

### 1. Zero-Regression Protocol (The Shield) 🛡️
...
### 2. Modular Integrity (The Silos) 🏗️
...
### 3. Core Architectural Pillars (The Mandates) 🏗️

- **Config-First Directive ⚙️:** NEVER hardcode API keys, URLs, or model settings. All configuration MUST be loaded from `.env` or `config.yaml`.
...
- **Resource & Token Management:** See detailed rules in **Section 9**.
- **Code Cleanliness:** See detailed rules in **Section 7**.

### 4. The Execution Life Cycle (The Surgical Path) 🔄
...
5. **QA Gate (Dual-Stack):**
   ...
   - If ANY gate fails, trigger the **Rollback Protocol** (see Section 10).
...
7. **Knowledge Injection (Definition of Done):** A task is considered "Done" ONLY after: (a) the **Sentinel** approves the QA Gate, (b) the **Researcher** updates Knowledge Items (KIs) or `.agent/MEMORY.md` with new findings, AND (c) a clean Git commit including ALL changes (code + documentation) is created. This ensures the repository state is never "Dirty" after a task completion.

### 5. Vibe Coding Skills & Toolset (Sub-Routines) 🕵️‍♂️
...
- **Android Best Practices & Quran Expert:** Mobile architecture blueprints and specialized app logic (`Android_Best_Practices`, `Android_Quran_Expert_Skills`, `quran_app_logic`).
- **Antfu Skills Ecosystem:** Vite, Vue, and Nuxt best practices (`Antfu_Skills`).
- **Standard Skills 2026 & Agent Skills:** Cutting-edge agent skill implementations and automation (`Standard_Skills_2026`, `agentskills-main`, `antigravity-skills-main`).
- **Cursor Architecture Rules:** Pre-defined system instructions for optimal IDE interaction (`Cursor_Architecture_Skills`, `cursor-rules-main`).
...
### 6. Agent Routing Map (The Chain of Command) 🦅
...
### 7. Coding Standards (The Clean Code Oath) 🧹
...
- If you are unsure, STOP and ask the Lead Engineer.
- All changes must be compliant with Section 9 (Modular Architecture Rules) of `GEMINI.md`.
- Ensure no PII or secrets are committed to version control.
- Verify `requirements.txt`, `pubspec.yaml`, and `Dockerfile` are in sync with the codebase.

### 9. Resource & Token Budget (The Sentinel's Watch) 💸
...
- **Rate Limiting:** Respect API rate limits for all external services (Gemini, Evolution API, Supabase). Use `tenacity` with jitter to avoid thundering herd.

### 10. Rollback Protocol (The Safety Net) 🔙
...
3. **Ghost Package Cleanup:** If libraries were changed, run `pip-sync` or `flutter clean && flutter pub get` to align the environment with the restored state.
4. **Catastrophic Rollback:** Run `git reset --hard HEAD`. If a feature branch was used: `git checkout <original-branch> && git branch -D <failed-branch>`.
5. **Stash Recovery:** Clear bad state first, then `git stash pop`.
6. **Post-Mortem:** Document the failure briefly before re-entry at Step 1.

### 11. Modular Data Isolation (The Tenant Shield) 🛡️
...
### 12. The Puzzle Strategy (Root Cause Analysis) 🧩
...
### 13. Standardized Artifacts (The Source of Truth) 📂
...
### 14. Automation Loop & Self-Optimization (Ralph Style) ⚡
...
- **Auto-Rollback:** If QA fails, immediately trigger Rule 10 (Rollback) and re-evaluate the approach.
