...
def register_all_routers(app: FastAPI):
    ...
    from backend.routers.evolution import router as evolution_router  # pylint: disable=import-outside-toplevel
    from backend.routers.users import router as users_router  # pylint: disable=import-outside-toplevel
    from backend.routers.license import router as license_router  # pylint: disable=import-outside-toplevel
    from backend.routers.analytics import router as analytics_router  # pylint: disable=import-outside-toplevel
    from backend.routers.system import router as system_router  # pylint: disable=import-outside-toplevel
    from backend.routers.intelligence import router as intelligence_router  # pylint: disable=import-outside-toplevel
    from backend.routers.connector import router as connector_router  # pylint: disable=import-outside-toplevel
    from backend.routers.crm import router as crm_router  # pylint: disable=import-outside-toplevel
    from backend.routers.memory import router as memory_router  # pylint: disable=import-outside-toplevel
    ...
    app.include_router(
        memory_router, prefix="/memory", tags=["Customer Memory"]
    )
    # G777 Legacy compatibility alias for Flutter Frontend
    app.include_router(evolution_router, prefix="/api/wa-hub", tags=["WhatsApp Hub"])
    app.include_router(system_router)

    # G777 Modules (Legacy Python UI Controllers - Temporarily Commented Out)
